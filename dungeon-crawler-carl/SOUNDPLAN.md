...
| `music_battle_c` (`battle_winter.ogg`) | **NO VERDICT YET — the owner has not heard the trim.** Opened by perf-mobile opt8, not by a complaint. | **OPEN — needs the ear.** The bed was cut 262.45s → 64.0s (`tools/audio/fix-beds.mjs battle_winter.ogg`): one 64.0s window of the A section, source 12.8s–78.4s, 40 bars at 150bpm, one-bar (1.6s) seam crossfade. What the instruments CAN say: seam delta 95.1dB → **0.4dB** with no click, inside the 0.1–1.5dB bound the six shipped band beds pass (§3); silence share 2% → 0%; peak -7.2 → -7.5 dBFS and 400ms-max RMS -20.4 → -20.9 dB, so the level is unmoved and the §2.2 headroom contract still holds. What they CANNOT say, and nobody in this loop can hear: whether **64s is long enough before the repeat becomes obvious**, and whether the A-section window is the right 64 seconds musically. The director drops this bed 6s after the last blow, so a typical exposure is well under one pass — but a long fight loops it. **If the owner says it repeats, the fix is a longer window, not a revert**: re-run the tool against the `main@b7c15f3` source with a larger `SPAN` (96s = 60 bars is the next macro-autocorrelation peak). Wire cost, for what the verdict is buying: 3,719,544 → 900,318 bytes, fetched MID-FIGHT in the 9.1fps scene. |
