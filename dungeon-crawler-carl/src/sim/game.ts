...
export function damagePlayerHit(
  ...
  opts: { dir?: Vec2; roll?: boolean; effect?: StatusKind; hazard?: boolean; melee?: boolean } = {},
): boolean {
...
function doPlayerAttack(state: GameState, p: Player, aim: Vec2, move: Vec2, arcMult = 1): void {
  ...
  smashBreakables(state, ({ pos }) => {
    ...
    return Math.hypot(to.x, to.y) <= mp.range + 0.25 && angleBetween(facing, to) <= arc / 2;
  });
...
function updateBarrage(state: GameState, p: Player, aim: Vec2, dt: number): void {
  ...
  state.strikes.push({
    pos: {
      x: at.x + Math.cos(a) * r + (perp.x / plen) * band,
      y: at.y + Math.sin(a) * r + (perp.y / plen) * band,
    },
    t: 0.35,
    ownerId: p.id,
    kind: "shell",
