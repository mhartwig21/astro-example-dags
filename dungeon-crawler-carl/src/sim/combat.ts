...
import { dacos, datan2, dcos, dhypot, dsin } from "./dmath";
...
export function turnToward(from: Vec2, to: Vec2, maxRad: number): Vec2 {
  const a0 = datan2(from.y, from.x);
  const a1 = datan2(to.y, to.x);
  let d = a1 - a0;
...
export function angleBetween(a: Vec2, b: Vec2): number {
  const am = dhypot(a.x, a.y);
  const bm = dhypot(b.x, b.y);
  if (am === 0 || bm === 0) return Math.PI;
  const dot = (a.x * b.x + a.y * b.y) / (am * bm);
  return dacos(Math.max(-1, Math.min(1, dot)));
}
