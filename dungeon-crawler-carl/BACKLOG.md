...
1. **Roam mode isn't saved/resumable yet.** `createGame`'s new `runKind`
...
2. **Finale borrows band escalations.** The floor-18 boss kept only its tier-3
...
3. **More shrine bargains.** The System Shrine ships with two deals + walk-away
...
4. **Status effects in the 2D host + character sheet.** The 5.13 status layer
...
5. **Status audio cues.** No sounds for ignite/poison/chill apply or DoT
   ...
   `HitEvent.effect`.
6. **Clown bombs on boss blast telegraphs.** The clown-ordnance prop renders on
...
7. **iPad touch controls (Wild Rift-style).** Full plan scoped 2026-07-08.
