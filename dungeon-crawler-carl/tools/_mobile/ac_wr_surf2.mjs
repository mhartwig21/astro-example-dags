...
function touchDriver(client) {
  ...
  return {
    ...
    async drag(x, y, tx, ty, opts = {}) {
      // `settle` holds the finger still 240ms before the lift so a scroll
      // drag ends dead, not as a fling — tapping a row that is still
      // decelerating is how the perf check manufactured its own FAIL
      // (ac_wr_perfrow.mjs is the settled reference; this matches it).
      const { id = 1, steps = 8, holdMs = 28, lift = true, settle = false } = opts;
    ...
    if (krow) {
      ...
      if (opts) {
        ...
        if (pm.on && pm.ok) {
          ...
          await page.waitForTimeout(500);
          let after = await page.evaluate(() => document.getElementById("kb-perfmode").textContent);
          if (after === before) { // one re-aimed tap, same allowance as ac_wr_perfrow
            const pm2 = await hit(page, "#kb-perfmode");
            if (pm2.ok) await touch.tap(pm2.cx, pm2.cy);
            await page.waitForTimeout(500);
