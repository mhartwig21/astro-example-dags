...
//
// Usage: node tools/acc1c_perf.mjs --adapter igpu|dgpu [--reps 3] [--secs 5]
...
try {
  ...
  async function ladder(sceneName, swing) {
    ...
    for (let r = 0; r < reps + 1; r++) {
      // ROTATE THE ORDER. A fixed low->medium->high charges the later modes for
      // whatever drifted since the first, and in this scene something DOES
      // drift: the crawler kills the pack it is standing in, so a fixed order
      // measures HIGH in the thinnest scene of every rep. Pass B did exactly
      // that and it flatters HIGH.
      ...
      for (const m of order) {
        ...
        // RE-STAGE BEFORE EVERY WINDOW. Same reason: "the worst real scene" has
        // to be the scene each window is actually taken in, not the scene the
        // first window was taken in.
        if (swing) {
          await page.evaluate(() => window.__toPack());
...
writeFileSync(`${OUT}/acc1c_${adapter}.json`, JSON.stringify(out, null, 1));
