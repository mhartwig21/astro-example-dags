...
try {
  for (const vp of [{ w: 1366, h: 768 }]) {
    ...
    await page.waitForTimeout(600); await (await page.context().newCDPSession(page)).send("Page.captureScreenshot",{format:"png"}); /* burn one lagged frame */
...
writeFileSync(`${OUT}/abil-probe2.json`, JSON.stringify(report, null, 1));
