...
**SHIPPED (r1, branch `tutorial`).** The design sections that became code are
deleted (BACKLOG.md convention); what remains is the enduring canon (the
two-voice rule, the register bible), the implementation map, and the open
...
- **`src/ui/guide.ts`** — the beat table as DATA + the `Guide` sequencer
  ...
  structural), `tut.runback` (B8, verdict aside plate), `tut.menu2` (B9,
  second organic check-in with ≥1 finished run). `tut.skipAll` is the global
  skip: every beat ledgered + the remaining onramp lines silenced.
...
- **Link arrivals** (`?daily=`/`?c=`/`?rush`/`?join=`/`?runback=`) skip
  ...
  and test mode construct no guide at all. `?clean=1` suppresses beats too.
...
- **Party onboarding** (`?join=`): all beats suppressed — the dialogue seam
  has no wire message for answers. Later phase.
